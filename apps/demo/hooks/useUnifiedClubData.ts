...
import type {
  StoredRegistration,
  StoredRegistrationTeam,
} from "./useNewRegistrationStorage";
...
function convertStoredToRegistrationDTOs(
...
function mergeClubData(demoData: ClubData): ClubData {
  ...
  const localRegisteredTeams: RegisteredTeamDTO[] = localRegistrations.flatMap(
    (stored) =>
...
export function getRegistrationById(
  ...
  const byParent = registrations.find(
    (r) => r._parentRegistrationId === registrationId,
  );
  ...
  const loadData = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const response = await fetch(
        `/api/demo/club-data?clubOwnerId=${DEFAULT_CLUB_OWNER_ID}`,
      );
      if (!response.ok) {
...
export function getLocalStorageRegistration(
  registrationId: string,
  ...
  return (
    registrations.find(
      (r) => r.id === registrationId || registrationId.startsWith(r.id),
    ) || null
  );
}
