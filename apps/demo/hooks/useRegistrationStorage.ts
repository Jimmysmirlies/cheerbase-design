...
export function useRegistrationStorage(registrationId: string) {
  ...
  const [savedChanges, setSavedChanges] =
    useState<StoredRegistrationChanges | null>(null);
  ...
  const saveChanges = useCallback(
    (
      changes: Omit<StoredRegistrationChanges, "submittedAt" | "pastInvoices">,
    ) => {
      const key = getStorageKey(registrationId);
      ...
      if (savedChanges) {
        ...
        pastInvoices = savedChanges.pastInvoices
  ...
  const clearChanges = useCallback(() => {
    ...
    try {
      ...
      console.error(
        "Failed to clear registration changes from storage:",
        error,
      );
      return false;
    }
