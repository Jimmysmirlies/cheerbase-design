...
export function updateUserTeam(
  userId: string,
  teamId: string,
  updates: Partial<Omit<StoredTeam, "id">>,
): StoredTeam | null {
  const teams = getUserTeams(userId);
...
export function updateUserRoster(
  userId: string,
  teamId: string,
  roster: Partial<Omit<StoredRoster, "teamId">>,
): StoredRoster | null {
  const rosters = getUserRosters(userId);
  ...
  const updatedRosters = rosters.map((r) =>
    r.teamId === teamId ? updatedRoster : r,
  );
...
export function useUserTeams(userId?: string) {
  ...
  const updateTeam = useCallback(
    (teamId: string, updates: Partial<Omit<StoredTeam, "id">>) => {
      if (!userId || userId === DEMO_CLUB_OWNER_ID) return null;
  ...
  const updateRoster = useCallback(
    (teamId: string, roster: Partial<Omit<StoredRoster, "teamId">>) => {
      if (!userId || userId === DEMO_CLUB_OWNER_ID) return null;
