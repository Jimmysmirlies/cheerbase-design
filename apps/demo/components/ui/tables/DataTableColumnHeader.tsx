...
export function DataTableColumnHeader({
  ...
  const allSelected = filterOptions.length > 0 && selectedFilters?.size === filterOptions.length
  ...
  return (
    <th className={cn('px-3 py-3 font-medium sm:px-4', className)}>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="min-w-56 max-w-80 overflow-hidden p-0 max-h-80">
          <ScrollArea className="!max-h-80">
            <div className="p-1">
                ...
                {filterable && filterOptions.length > 0 && <DropdownMenuSeparator />}
                  ...
                  <>
                    ...
                    <div className="p-2">
                      {filterOptions.map((option) => {
                        const isChecked = selectedFilters?.has(option.value) ?? false
                        return (
                          <div
                            key={option.value}
                            role="button"
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && handleFilterToggle(option.value)}
                            className="flex cursor-pointer items-center justify-between gap-2 rounded-sm px-2 py-1.5 hover:bg-accent"
                          >
