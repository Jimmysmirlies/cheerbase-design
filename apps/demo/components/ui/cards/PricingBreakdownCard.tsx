...
export function PricingBreakdownPanel({
  ...
  const pricingSummary = useMemo(() => {
    ...
    return Object.entries(entriesByDivision).map(
      ([divisionName, divisionEntries]) => {
  ...
  const hasUnpricedDivision = pricingSummary.some(
    (item) => !item.hasPricing && item.participants > 0,
  );
    ...
    <div className="space-y-3 body-text">
      ...
      {hasUnpricedDivision && (
        <p className="body-text text-amber-600">
          Pricing is still pending for at least one division and is excluded
          from the total.
        </p>
      )}
      <div className="border-border/60 text-foreground space-y-2 border-t pt-3">
        ...
        <div className="flex items-center justify-between body-text">
          ...
          <span>
            {formatCurrency(
              activeTaxSummary.baseAmount *
                (activeTaxSummary.gstRate + activeTaxSummary.qstRate),
            )}
          </span>
        </div>
        <div className="flex items-center justify-between">
          ...
          <span className="body-large font-semibold">
            {formatCurrency(
              activeTaxSummary.baseAmount *
                (1 + activeTaxSummary.gstRate + activeTaxSummary.qstRate),
            )}
          </span>
        </div>
      </div>
