...
import {
  TextSelect,
  type TextSelectSection,
} from "@workspace/ui/components/text-select";
...
export function InvoiceView({
  ...
  const pricingByDivision = useMemo(() => {
    return divisionPricing.reduce<Record<string, DivisionPricing>>(
      (acc, option) => {
    ...
    return Object.entries(entriesByDivision).map(([divisionName, entries]) => {
      ...
      if (!pricing) {
        return {
          ...
          qty,
          originalQty:
            changeStatus === "modified" && originalQty !== qty
              ? originalQty
              : undefined,
          unit: 0,
          lineTotal: 0,
  ...
  const invoiceSelectSections: TextSelectSection[] | undefined =
    hasMultipleInvoices
      ? [
          {
            label: "Current",
                ...
                >
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <span
                        className={
                          item.changeStatus === "removed" ? "line-through" : ""
                        }
                      >
                        {item.category}
                      </span>
                      {item.changeStatus === "new" && (
                        <span className="label text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-1.5 py-0.5 rounded">
                          New
                        </span>
                      )}
                      {item.changeStatus === "modified" && (
                        <span className="label text-amber-600 dark:text-amber-400 bg-amber-100 dark:bg-amber-900/30 px-1.5 py-0.5 rounded">
                          Modified
                        </span>
                      )}
                      {item.changeStatus === "removed" && (
                        <span className="label text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 px-1.5 py-0.5 rounded">
                          Removed
                        </span>
                      )}
                    </div>
                  ...
                  <td
                    className={`py-3 text-right ${item.changeStatus === "removed" ? "line-through" : ""}`}
                  >
                    {item.qty}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-3">
            {payments.map((payment, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between body-text"
              >
                <div className="flex items-center gap-2">
