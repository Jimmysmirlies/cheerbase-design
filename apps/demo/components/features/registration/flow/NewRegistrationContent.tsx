...
export function NewRegistrationContent({
  ...
  const invoiceData = useMemo(() => {
    ...
    registeredTeams.forEach(team => {
      ...
      const unitPrice = pricing?.regular?.price ?? pricing?.earlyBird?.price ?? 0
  ...
  const isTeamAlreadyRegistered = useCallback((teamId: string) => {
    return registeredTeams.some(t => t.id === teamId || t.detailId === teamId)
  }, [registeredTeams])

  // Handle adding a team from RegisterTeamModal
  const handleRegisterTeam = useCallback((entry: RegistrationEntry) => {
  ...
  // Handle bulk import
  const handleBulkImport = useCallback((entries: RegistrationEntry[]) => {
  ...
  // Handle roster save
  const handleSaveRoster = useCallback((members: RegistrationMember[]) => {
  ...
  const handleSubmitRegistration = useCallback(() => {
    ...
    const redirectPath = confirmationPath ?? `/events/${encodeURIComponent(eventId)}/register/confirmation?registrationId=${encodeURIComponent(savedRegistration.id)}`
    router.push(redirectPath)
  }, [registeredTeams, eventName, eventId, organizer, organizerGradient, eventDate, location, registrationDeadline, invoiceData, confirmationPath, router])
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <WalkthroughSpotlight step="register-team" side="bottom" align="end" advanceOnClick>
          ...
          <div className="rounded-md border border-border/60 bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
            ...
            {registrationDeadline ? (
              <> Registration must be completed before <span className="font-medium text-foreground">{registrationDeadline}</span>.</>
            ) : null}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6 min-w-0">
          {allDivisions.length > 0 ? (
            ...
            <div className="flex min-h-[120px] items-center justify-center rounded-md border border-dashed border-border/60 bg-muted/10 px-5 py-4">
              <p className="text-sm text-muted-foreground">No divisions available for this event</p>
            </div>
          )}
        </div>
      </div>
          ...
          <CardContent className="flex flex-col gap-4 px-6 py-0">
                ...
                {invoiceData.items.map((item) => (
                  <div key={item.id} className="flex flex-col gap-1">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium text-foreground">{item.division}</span>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-foreground">{formatCurrency(invoiceData.subtotal)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Tax (15%)</span>
                <span className="text-sm text-foreground">{formatCurrency(invoiceData.tax)}</span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xl font-semibold text-foreground">{formatCurrency(invoiceData.total)}</span>
            </div>
        ...
        <div className="flex items-center justify-between gap-4 px-4 py-3">
          <div className="flex flex-col">
            <p className="text-sm font-semibold text-foreground">
              {registeredTeams.length} team{registeredTeams.length === 1 ? '' : 's'} · {formatCurrency(invoiceData.total)}
            </p>
            <p className="text-xs text-muted-foreground">
              {registeredTeams.length === 0 ? 'Add teams to register' : 'Ready to submit'}
            </p>
          </div>
          <Button 
            size="sm" 
