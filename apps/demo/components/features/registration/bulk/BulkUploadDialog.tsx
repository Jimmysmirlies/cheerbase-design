...
export function BulkUploadDialog({
  ...
  const [step, setStep] = useState<"upload" | "map" | "preview" | "confirm">(
    "upload",
  );
  ...
  const handleFile = useCallback(async (file?: File) => {
    try {
      ...
      if (ext !== "csv") {
        setError(
          "Only CSV is supported in this demo. Please export your Excel file as CSV.",
        );
        return;
      }
  ...
  const entriesByDivision = useMemo(() => {
    ...
    groups.forEach((group) => {
      ...
      const existing = teamOptions.find(
        ...
        <div className="flex flex-1 flex-col gap-6 overflow-y-auto px-6 pb-6">
          {step === "upload" ? (
            <section className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                Tip: Export your Excel file as CSV. Template:{" "}
                <a
                  className="underline"
                  href="/templates/registration-template.csv"
                >
          ...
          {step === "preview" && groups.length ? (
            <section className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Preview grouped teams. Existing teams are auto-linked by name.
              </p>
                  ...
                  <tbody>
                    {groups.map((g) => {
                      const match = teamOptions.find(
                        (t) =>
                          t.name.toLowerCase() === g.teamName.toLowerCase(),
                      );
                      return (
                        <tr
                          key={`${g.teamName}:${g.division}`}
                          className="border-t"
                        >
                          ...
                          <td className="px-3 py-2">
                            {match ? `Linked (${match.name})` : "New team"}
                          </td>
                        </tr>
                      );
                    })}
