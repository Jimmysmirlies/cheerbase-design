...
export function TeamCard({ team, isEditMode = false, onEdit, isLoading = false }: TeamCardProps) {
  ...
  const handleDebugClick = (e: React.MouseEvent) => {
    ...
    if (h3 && contentDiv && flexContainer && outerCard) {
      ...
      console.log('  - Text overflow visible:', h3.scrollWidth - h3.offsetWidth, 'px')
      console.log('  - Content div can shrink?', getComputedStyle(contentDiv).minWidth === '0px')
      console.log('  - H3 can shrink?', getComputedStyle(h3).minWidth === '0px')
      console.log('  - Flex container can shrink?', getComputedStyle(flexContainer).minWidth === '0px')
      ...
      while (parent && level <= 8) {
        ...
        const hasWidthConstraint = styles.width !== 'auto' || styles.minWidth !== '0px' || styles.maxWidth !== 'none'
        console.log(`${level}️⃣ ${parent.className.slice(0, 60)}${parent.className.length > 60 ? '...' : ''}`, {
          width: styles.width,
  ...
  return (
    <div className={cn(
      ...
      >
        ...
        <div className={cn("min-w-0 flex-1", debugMode && "outline outline-2 outline-blue-500")}>
          <h3 className={cn("heading-4 text-foreground line-clamp-2", debugMode && "outline outline-2 outline-red-500")}>{team.name}</h3>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          >
            <ChevronDownIcon className={cn('size-4 sm:size-5 transition-transform', expanded && 'rotate-180')} aria-hidden="true" />
          </Button>
        </div>
      </div>
