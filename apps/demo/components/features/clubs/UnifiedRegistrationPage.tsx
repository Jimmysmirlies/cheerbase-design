...
export function UnifiedRegistrationPage({ registrationId }: UnifiedRegistrationPageProps) {
  ...
  let invoiceLineItems: { category: string; unit: number; qty: number; lineTotal: number }[]
  let subtotal: number
  let totalTax: number
  ...
  if (localStorageReg) {
    ...
    invoiceLineItems = Array.from(teamsByDivision.entries()).map(([division, teams]) => {
      const memberCount = teams.reduce((sum, t) => sum + (t.members?.length ?? 0), 0)
      const pricing = divisionPricing.find(d => d.name === division)
      const unitPrice = pricing?.regular?.price ?? pricing?.earlyBird?.price ?? 130
      return {
        category: division,
        unit: unitPrice,
        qty: memberCount,
