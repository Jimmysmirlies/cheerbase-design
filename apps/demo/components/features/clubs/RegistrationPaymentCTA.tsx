...
const statusStyles: Record<PaymentStatus, { container: string; title: string; description: string }> = {
  paid: {
...
export function RegistrationPaymentCTA({
  ...
  return (
    <>
      <div className={cn('rounded-md border px-4 py-4 body-text', styles.container)}>
        <div className={cn('flex flex-col gap-3', !hideButtons && 'sm:flex-row sm:items-center sm:justify-between')}>
          <div className="flex flex-col gap-1">
            <p className={cn('font-semibold flex items-center gap-2', styles.title)}>
              {status === 'paid' && <CheckCircle2Icon className="size-4" />}
              {title}
              {status === 'paid' && paidAtLabel ? (
                <span className={cn('font-normal', styles.description)}>{` · ${paidAtLabel}`}</span>
              ) : status !== 'paid' && dueLabel ? (
                <span className={cn('font-normal', styles.description)}>{` · Due ${dueLabel}`}</span>
              ) : null}
            </p>
          ...
          {!hideButtons && (
            <div className="flex items-center gap-2 shrink-0">
              <Button asChild size="sm" variant={status === 'paid' ? 'default' : 'outline'}>
