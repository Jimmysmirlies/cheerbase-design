...
import {
  buildInvoiceDataFromRegistration,
} from '@/lib/invoices'
...
function buildLocalStorageInvoice(
  ...
  const divisionPricing: DivisionPricingItem[] = (event?.availableDivisions ?? []).map(d => ({
    name: d.name,
  ...
  if (divisionPricing.length === 0) {
    const uniqueDivisions = new Set(localStorageReg.teams.map(t => t.division))
  ...
  const invoice: InvoiceData = {
    ...
    entriesByDivision,
    divisionPricing,
    payments: localStorageReg.status === 'paid' 
      ? [{
          amount: localStorageReg.total,
          method: 'Visa',
          lastFour: '4242',
          date: localStorageReg.paidAt ? new Date(localStorageReg.paidAt) : new Date(localStorageReg.invoiceDate),
        }]
      : [],
...
export function UnifiedInvoicePage({ registrationId }: UnifiedInvoicePageProps) {
  ...
  const originalPaymentStatus = registration.status === 'paid' ? 'paid' : 'unpaid'
