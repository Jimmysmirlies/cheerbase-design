...
export function RegistrationSummaryCard({
  ...
  const [eventDateParts, setEventDateParts] = useState<{
    month: string;
    day: string;
    weekday: string;
    fullDate: string;
  ...
  useEffect(() => {
    ...
    if (eventDateObj && !Number.isNaN(eventDateObj.getTime())) {
      setEventDateParts({
        ...
        fullDate: eventDateObj.toLocaleDateString("en-US", {
          day: "numeric",
          month: "long",
        }),
      });
        ...
        <>
                ...
                >
                  {isRegistrationClosed ? (
                    ...
                    <TooltipProvider delayDuration={0}>
                      <Tooltip>
                        ...
                        <TooltipContent side="bottom">
                          Log in as a Club Owner account to register for this
                          event
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                    ...
                    <Button asChild className="w-full">
                      <Link
                        href={`/events/${encodeURIComponent(eventId)}/register`}
                      >
                        {registerButtonLabel}
                      </Link>
                    </Button>
          ...
          <AuthDialog
            ...
            onDemoLogin={(nextRole) => {
              const demoId =
                nextRole === "club_owner" ? "club-owner-1" : "organizer-demo-1";
              signInAsRole(
                nextRole,
                nextRole === "club_owner"
                  ? "Demo Club Owner"
