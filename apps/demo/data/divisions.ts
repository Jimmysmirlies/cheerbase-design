...
> = {
  [eventDivisionNames.worlds]: {
    label: "Senior / Open",
    subtitle: "Ages 15+",
    before: 65,
    after: 85,
  },
  [eventDivisionNames.chaperone]: {
    label: "Accompanist / Chaperone",
    subtitle: "Non-competing attendees",
    before: 40,
    after: 40,
  },
  [eventDivisionNames.stuntIndyDuo]: {
    label: "Stunt / Individual / Duo",
    subtitle: "Solo and partner events",
    before: 45,
    after: 45,
  },
  [eventDivisionNames.prepNovice]: {
    label: "Initiation / Prep / Novice",
    subtitle: "Ages 4-8",
    before: 40,
    after: 60,
  },
  [eventDivisionNames.allStarScholastic]: {
    label: "Youth / Junior",
    subtitle: "Ages 9-14",
    before: 55,
    after: 75,
  },
  [eventDivisionNames.adaptive]: {
    label: "Adaptive / Cheer Abilities",
    subtitle: "Inclusive divisions",
    before: 0,
    after: 0,
  },
};
...
export const divisionIndex = divisionCatalog.reduce<Record<string, string[]>>(
  (acc, category) => {
...
export const divisionFullNames: string[] = divisionCatalog.flatMap((category) =>
  category.tiers.flatMap((tier) =>
    tier.levels.map((level) => `${category.name} - ${tier.name} - ${level}`),
  ),
);
