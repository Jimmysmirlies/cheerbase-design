...
import { getEventsByOrganizerId, findOrganizerById, listEvents } from './selectors'
 ...
 */
function formatInvoiceNumber(registrationId: string, eventId: string, version = 1): string {
...
export function getRegistrationTrend(organizerId: string): MonthlyData[] {
  ...
  const monthMap = new Map<string, { registrations: number; revenue: number; participants: number }>()

  for (const reg of registrations) {
    const submittedDate = getSubmittedDate(reg)
    const monthKey = formatMonthKey(submittedDate)

    const existing = monthMap.get(monthKey) ?? { registrations: 0, revenue: 0, participants: 0 }
...
export function getEventPerformance(organizerId: string): EventPerformance[] {
  ...
  for (const event of events) {
    ...
    const unpaidPercentage = regs.length > 0 ? (unpaidCount / regs.length) * 100 : 0

    // Flag events that need attention: >50% unpaid or overdue payments
    const needsAttention = unpaidPercentage > 50 || revenueOutstanding > revenuePaid
 ...
 */
export function getRegistrationTableData(organizerId: string): RegistrationTableRow[] {
  const registrations = getOrganizerRegistrations(organizerId)
  const now = new Date()
