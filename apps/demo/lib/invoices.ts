...
import { groupEntriesByDivision, getEntryMemberCount } from '@/utils/registration-stats'
...
function mapMembersToRegistrationMembers(members: RegisteredMemberDTO[] | undefined): RegistrationMember[] | undefined {
...
function normalizeDivisionName(division: string, event?: Event | null) {
  ...
  const matched = event.availableDivisions.find(option => option.name.toLowerCase() === normalized)
  return matched?.name ?? baseName
}

function resolveRegisteredTeam(registration: RegistrationDTO, clubData: ClubData): RegisteredTeamDTO | null {
  return (
    registration.registeredTeam ??
    (registration.registeredTeamId
      ? clubData.registeredTeams.find(rt => rt.id === registration.registeredTeamId)
      : null) ??
    (registration.teamId ? clubData.registeredTeams.find(rt => rt.sourceTeamId === registration.teamId) : null) ??
    null
  )
}

function buildMembersFromRoster(roster?: ClubData['rosters'][number] | null): RegisteredMemberDTO[] {
  if (!roster) return []
...
function buildInvoiceTeamEntryFromRegistration(
  ...
  const roster = registration.teamId ? clubData.rosters.find(r => r.teamId === registration.teamId) : null
  const teamRecord = registration.teamId ? clubData.teams.find(team => team.id === registration.teamId) : undefined

  const rosterMembers = buildMembersFromRoster(roster)
  let members = registeredTeam?.members?.length ? registeredTeam.members : rosterMembers

  if ((!members || members.length === 0) && typeof registration.athletes === 'number' && registration.athletes > 0) {
  ...
  const teamName = registeredTeam?.name ?? teamRecord?.name ?? registration.division
  ...
  return {
    ...
    division,
    teamName,
    teamId: registration.teamId ?? registeredTeam?.sourceTeamId ?? registeredTeam?.id,
    registeredTeamId: registeredTeam?.id ?? registration.registeredTeamId ?? null,
    members: members ?? [],
...
function deriveIssuedDate(registration: RegistrationDTO) {
  const candidates = [registration.snapshotTakenAt, registration.paymentDeadline, registration.eventDate]
...
function ensureDivisionPricing(
  ...
  const pricingByName = new Map(basePricing.map(option => [option.name, option]))
  const participantsByDivision = entries.reduce<Record<string, number>>((acc, entry) => {
  ...
  Object.entries(participantsByDivision).forEach(([division, participantCount]) => {
    if (pricingByName.has(division)) return
    const safeParticipants = participantCount > 0 ? participantCount : 1
    const derivedUnitPrice = invoiceTotalNumber > 0 ? invoiceTotalNumber / safeParticipants : 0

    ensuredPricing.push({
      name: division,
      regular: { price: Number.isFinite(derivedUnitPrice) ? derivedUnitPrice : 0 },
    })
  })
...
export function convertInvoiceTeamsToRegistrationEntries(entries: InvoiceTeamEntry[]): RegistrationEntry[] {
...
export function buildInvoiceDataFromEntries(
  ...
  const invoiceId = normalizeInvoiceId(options?.invoiceId, `${registration.id}:${registration.eventId}`)
  const invoiceNumber = formatInvoiceNumber(invoiceId, orderVersion)

  const entriesByDivision = groupEntriesByDivision(entries)
  const divisionPricing = ensureDivisionPricing(event?.availableDivisions ?? [], entries, registration)
      ...
      ? [
          {
            ...
            method: 'Visa',
            lastFour: '4242',
            date: registration.paidAt ? new Date(registration.paidAt) : issuedDate,
          },
        ]
      : []
...
export function buildInvoiceDataFromRegistration(
  ...
  const invoiceTeams = buildInvoiceTeamEntries(clubData, registration.eventId, event, {
    scope: 'event',
    registrationId: registration.id,
  })
  const entries = convertInvoiceTeamsToRegistrationEntries(invoiceTeams)
  return buildInvoiceDataFromEntries(entries, registration, event ?? null, clubData, options)
}

export function calculateInvoiceTotals(invoice: InvoiceData) {
  const pricingByDivision = invoice.divisionPricing.reduce<Record<string, DivisionPricing>>((acc, option) => {
  ...
  const lineItems = Object.entries(invoice.entriesByDivision).map(([divisionName, entries]) => {
