...
function SearchEventsPageContent() {
  ...
  const organizerOptionsWithAll = useMemo(
    () => [
      ...
      ...organizerNames.map((name) => ({ value: name, label: name })),
    ],
    [organizerNames],
  );
  ...
  const filteredEvents = useMemo(() => {
    ...
    const base = [...events].filter((event) => {
      ...
      const matchesTerm =
        term.length === 0 ||
        `${event.name} ${event.organizer} ${event.location} ${divisionNames}`
          .toLowerCase()
          .includes(term);
          ...
          <>
            ...
            >
              <GlassSelect
                ...
                options={provinceOptionsWithAll.map((opt) => ({
                  value: opt.code,
                  label: opt.label,
                }))}
            ...
            >
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground text-sm sm:text-base">
                  Find events by title, organizer, or location. Refine with
                  division categories and organizer filters.
                </p>
              </div>
              <LayoutToggle
                variant={layoutVariant}
