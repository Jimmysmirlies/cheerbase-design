...
import { findEventByIdIncludingDrafts, isRegistrationClosed } from '@/data/events'
...
export default function OrganizerEventViewPage() {
  ...
  useEffect(() => {
    ...
    const handleSettingsChange = (settingsEvent: CustomEvent<{ gradient: string }>) => {
  ...
  const gradientKey: BrandGradient = organizerGradient ?? organizer?.gradient ?? 'teal'
