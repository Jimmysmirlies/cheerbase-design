...
export type BulkTeam = {
  teamName: string
  division: string
  members: Array<Pick<BulkMemberRow, 'name' | 'role' | 'dob' | 'email' | 'phone'>>
}
...
export function parseCsvText(text: string): { headers: string[]; rows: Record<string, string>[] } {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const hasAny = Object.values(row).some(v => String(v ?? '').trim().length)
...
export function groupRowsByTeam(rows: Record<string, string>[], mapping: BulkMapping): BulkTeam[] {
  const key = (r: Record<string, string>) => `${(r[mapping.teamName] || '').trim()}__${(r[mapping.division] || '').trim()}`
